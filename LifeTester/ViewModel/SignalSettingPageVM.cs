    ...
    public class SignalSettingPageVM : BaseVM
    {
        ...
        public int Duration
        {
            ...
            set
            {
                if (duration != value)
                {
                    ...
                    Cache.Instance.Cards.ForEach(c =>
                        {
                            c.Duration = value * 60 + durationHour*3600;
                            c.Channels.ForEach(h => h.Duration = value * 60 + durationHour*3600);
                        });
        ...
        public int DurationHour
        {
            ...
            set 
            {
                ...
                {
                    durationHour = value;
                    this.OnPropertyChanged(p => p.DurationHour);
                    Cache.Instance.Cards.ForEach(c =>
                        {
                            c.Duration = value * 3600 + duration * 60;
                            c.Channels.ForEach(h => h.Duration = value * 3600 + duration * 60);
                        }
                        );
