...
namespace LifeTester.Util
{
    public class NPOIHelper
    {
        ...
        static MemoryStream ExportDT(DataTable dtSource, string strHeaderText)
        {
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    ...
                    {
                        HSSFRow headerRow = sheet.CreateRow(startRowIndex) as HSSFRow;
                        HSSFCellStyle headStyle = workbook.CreateCellStyle() as HSSFCellStyle;
                        headStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                        HSSFFont font = workbook.CreateFont() as HSSFFont;
        ...
        static void ExportDTI(DataTable dtSource, string strHeaderText, FileStream fs)
        {
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                if (rowIndex == 0)
                {
                    ...
                    {
                        XSSFRow headerRow = sheet.CreateRow(startRowIndex) as XSSFRow;
                        XSSFCellStyle headStyle = workbook.CreateCellStyle() as XSSFCellStyle;
                        headStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                        XSSFFont font = workbook.CreateFont() as XSSFFont;
                        font.FontHeightInPoints = 10;
        ...
        private static MemoryStream ExportDataSetToExcel(DataSet sourceDs, List<string> sheetNames)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
        ...
        /// <param name="sheetNames">工作表名称集合</param>
        /// <returns>Excel工作表</returns>
        private static MemoryStream ExportDataSetToExcelI(DataSet sourceDs, List<string> sheetNames)
        {
            XSSFWorkbook workbook = new XSSFWorkbook();
            MemoryStream ms = new MemoryStream();
            for (int i = 0; i < sheetNames.Count; i++)
        ...
        /// <param name="sheetNames">sheet页名称</param>
        /// <param name="strFileName">保存位置</param>
        public static void ExportDStoExcel(DataSet ds, List<string> sheetNames, string strFileName)
        {
            string[] temp = strFileName.Split('.');

            if (temp[temp.Length - 1] == "xls")
            {
                using (MemoryStream ms = ExportDataSetToExcel(ds, sheetNames))
                {
                    using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                    {
                        byte[] data = ms.ToArray();
            ...
            {
                using (MemoryStream ms = ExportDataSetToExcelI(ds, sheetNames))
                {
                    using (FileStream fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                    {
                        byte[] data = ms.ToArray();
